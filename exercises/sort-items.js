...
function sortData(direction) {
  // Defines the container variable
  const container = document.getElementById("main");

  // Convert all item class into an array.
  const allItems = Array.from(container.querySelectorAll(".item"));

  // Sort through allItems
  allItems.sort((a, b) => {
...
sortBtn.forEach((item) => {
  ...
  item.addEventListener("click", () => {
    // Get the sort direction from the item's dataset property
    const sortDirection = item.dataset.sortdir;

    // Call the sortData function with the sort direction as the argument
